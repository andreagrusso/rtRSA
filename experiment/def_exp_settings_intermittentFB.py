...
block_duration = baseline_duration + task_duration + feedback_duration
...
feedbacks = np.array([baseline_duration + task_duration + 1,baseline_duration + task_duration + feedback_duration]).reshape(1,-1)
...
for im in conditions:
    ...
    protocol.save(os.path.join(outdir, 'intermittent', im_name +'_RSA_incremental_task.prt')) 

    
f=open(os.path.join(outdir,'intermittent','baselines.pkl'),'wb')
pickle.dump(baselines,f)


f=open(os.path.join(outdir,'intermittent','tasks.pkl'),'wb')
pickle.dump(tasks,f)


f=open(os.path.join(outdir,'intermittent','feedbacks.pkl'),'wb')
